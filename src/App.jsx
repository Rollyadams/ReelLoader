...
const NEWS_TOPICS = [
  { id: "cctv", label: "AI + Security", query: "AI security surveillance cameras smart 2025", emoji: "📷", color: "#e74c3c" },
  { id: "solar", label: "AI + Solar & Energy", query: "AI solar energy power management 2025", emoji: "⚡", color: "#f39c12" },
  { id: "webdev", label: "AI + Web & Apps", query: "AI web development tools automation 2025", emoji: "💻", color: "#3498db" },
  { id: "general", label: "Hot AI News", query: "artificial intelligence latest breakthrough 2025", emoji: "🤖", color: "#2ecc71" },
];
...
function drawConfusedGuy(ctx, x, y, size, speaking, dim) {
  ...
  roundRect(ctx, 30, 70, 60, 46, 10);
  ctx.fill();

  // Shirt white
  ctx.fillStyle = "#ffffff";
  roundRect(ctx, 48, 70, 11, 20, 2); ctx.fill();
  roundRect(ctx, 61, 70, 11, 20, 2); ctx.fill();

  // Tie
  ctx.fillStyle = "#2ecc71";
  ctx.beginPath(); ctx.moveTo(60, 73); ctx.lineTo(55, 87); ctx.lineTo(60, 92); ctx.lineTo(65, 87); ctx.closePath(); ctx.fill();

  // Neck
  ctx.fillStyle = "#f0b97a";
  roundRect(ctx, 52, 57, 16, 15, 5); ctx.fill();
  ...
  ctx.beginPath(); ctx.ellipse(60, 17, 27, 11, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillRect(33, 17, 54, 13);

  // Eyes
  ctx.fillStyle = "white";
  ctx.beginPath(); ctx.ellipse(49, 42, 6.5, speaking ? 7.5 : 5.5, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(71, 42, 6.5, speaking ? 7.5 : 5.5, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "#1a1a2e";
  ctx.beginPath(); ctx.arc(50.5, 43, 3.2, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(72.5, 43, 3.2, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "white";
  ctx.beginPath(); ctx.arc(51.5, 41.5, 1.1, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(73.5, 41.5, 1.1, 0, Math.PI * 2); ctx.fill();
  ...
  if (speaking) {
    ...
    ctx.beginPath(); ctx.ellipse(60, 58.5, 7.5, 5.5, 0, 0, Math.PI * 2); ctx.fill();
  } else {
    ctx.strokeStyle = "#c0392b"; ctx.lineWidth = 2.5;
    ctx.beginPath(); ctx.moveTo(52, 58); ctx.quadraticCurveTo(60, 64, 68, 58); ctx.stroke();
  }

  // Ears
  ctx.fillStyle = "#dfa060";
  ctx.beginPath(); ctx.ellipse(33, 44, 5, 7.5, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(87, 44, 5, 7.5, 0, 0, Math.PI * 2); ctx.fill();

  // Arms
  ctx.fillStyle = "#1e3a5f";
  roundRect(ctx, 12, 72, 18, 9, 4.5); ctx.fill();
  roundRect(ctx, 90, 72, 18, 9, 4.5); ctx.fill();

  // Question mark bubble
  if (!speaking) {
    ctx.fillStyle = "white";
    ctx.beginPath(); ctx.arc(96, 20, 15, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "#1e3a5f"; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(96, 20, 15, 0, Math.PI * 2); ctx.stroke();
    ctx.fillStyle = "#1e3a5f";
...
function drawSmartGuy(ctx, x, y, size, speaking, dim) {
  ...
  roundRect(ctx, 28, 70, 64, 46, 10); ctx.fill();

  // Pocket
  ctx.fillStyle = "#0a2040";
  roundRect(ctx, 33, 77, 20, 16, 4); ctx.fill();
  ...
  ctx.beginPath(); ctx.ellipse(60, 16, 26, 10, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillRect(34, 16, 52, 12);

  // Glasses frames
  ctx.strokeStyle = "#1a1a2e"; ctx.lineWidth = 2.2;
  roundRect(ctx, 41, 37, 15, 11, 4.5); ctx.stroke();
  roundRect(ctx, 64, 37, 15, 11, 4.5); ctx.stroke();
  ...
  // Eyes
  ctx.fillStyle = "white";
  ctx.beginPath(); ctx.ellipse(48.5, 42.5, 4.5, speaking ? 5.5 : 4, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(71.5, 42.5, 4.5, speaking ? 5.5 : 4, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "#1a1a2e";
  ctx.beginPath(); ctx.arc(49.5, 43, 2.8, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(72.5, 43, 2.8, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "white";
  ctx.beginPath(); ctx.arc(50.5, 41.5, 0.9, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(73.5, 41.5, 0.9, 0, Math.PI * 2); ctx.fill();
  ...
  if (speaking) {
    ...
    ctx.beginPath(); ctx.ellipse(60, 57, 7, 4.5, 0, 0, Math.PI * 2); ctx.fill();
  } else {
    ctx.strokeStyle = "#7b1f1f"; ctx.lineWidth = 2.5;
    ctx.beginPath(); ctx.moveTo(52, 56); ctx.quadraticCurveTo(60, 61, 68, 56); ctx.stroke();
  }

  // Ears
  ctx.fillStyle = "#b36840";
  ctx.beginPath(); ctx.ellipse(34, 43, 5, 7.5, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(86, 43, 5, 7.5, 0, 0, Math.PI * 2); ctx.fill();

  // Arms
  ctx.fillStyle = "#0f3460";
  roundRect(ctx, 10, 72, 18, 9, 4.5); ctx.fill();
  roundRect(ctx, 92, 72, 18, 9, 4.5); ctx.fill();

  // Lightbulb
  if (speaking) {
    ctx.fillStyle = "#fff9e0";
    ctx.beginPath(); ctx.arc(96, 18, 15, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "#f39c12"; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(96, 18, 15, 0, Math.PI * 2); ctx.stroke();
...
function drawFrame(canvas, script, currentLine, displayLine, selectedNews, topicEmoji, mouthOpen) {
  ...
  ctx.fillText("Rollyadams Techworld", W / 2, 52);

  // News banner
  if (selectedNews?.title) {
    ctx.fillStyle = "rgba(46,204,113,0.12)";
    roundRect(ctx, 20, 68, W - 40, 56, 12); ctx.fill();
    ctx.strokeStyle = "rgba(46,204,113,0.3)"; ctx.lineWidth = 1.5;
    roundRect(ctx, 20, 68, W - 40, 56, 12); ctx.stroke();
    ctx.fillStyle = "#4ade80";
    ctx.font = "bold 18px 'Nunito', sans-serif";
    ctx.textAlign = "center";
    const title = (topicEmoji + " " + (selectedNews.title || "")).slice(0, 55) + "...";
    ctx.fillText(title, W / 2, 102);
  }

  // Characters
  const charY = H * 0.48;
  const charSize = 140;
  const guy1Speaking = currentLine >= 0 && script[currentLine]?.speaker === 0 && mouthOpen;
  const guy2Speaking = currentLine >= 0 && script[currentLine]?.speaker === 1 && mouthOpen;
  const guy1Dim = currentLine >= 0 && script[currentLine]?.speaker === 1;
  const guy2Dim = currentLine >= 0 && script[currentLine]?.speaker === 0;

  drawConfusedGuy(ctx, W * 0.27, charY, charSize, guy1Speaking, guy1Dim);
  drawSmartGuy(ctx, W * 0.73, charY, charSize, guy2Speaking, guy2Dim);

  // Labels
  ctx.font = "bold 16px 'Nunito', sans-serif";
  ctx.textAlign = "center";
  ctx.fillStyle = "#4b6a8a";
  ctx.fillText("Guy 1", W * 0.27, charY + charSize * 0.5 + 18);
  ctx.fillText("Guy 2", W * 0.73, charY + charSize * 0.5 + 18);

  // Divider
  const grad2 = ctx.createLinearGradient(W / 2, charY - 40, W / 2, charY + 40);
  grad2.addColorStop(0, "transparent");
  grad2.addColorStop(0.5, "rgba(46,204,113,0.35)");
  grad2.addColorStop(1, "transparent");
  ctx.strokeStyle = grad2; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(W / 2, charY - 40); ctx.lineTo(W / 2, charY + 40); ctx.stroke();

  // Subtitle bar
  if (displayLine && currentLine >= 0) {
    const barY = H - 140;
    ctx.fillStyle = "rgba(8,20,40,0.93)";
    roundRect(ctx, 16, barY, W - 32, 90, 14); ctx.fill();
    ctx.strokeStyle = "rgba(255,255,255,0.08)"; ctx.lineWidth = 1;
    roundRect(ctx, 16, barY, W - 32, 90, 14); ctx.stroke();

    const speakerLabel = script[currentLine]?.speaker === 0 ? "● GUY 1" : "● GUY 2";
    const speakerColor = script[currentLine]?.speaker === 0 ? "#7dd3fc" : "#4ade80";
    ctx.fillStyle = speakerColor;
    ctx.font = "bold 14px 'Nunito', sans-serif";
    ctx.textAlign = "center";
    ctx.fillText(speakerLabel, W / 2, barY + 22);

    ctx.fillStyle = "#f8fafc";
    ctx.font = "bold 20px 'Nunito', sans-serif";
    ctx.textAlign = "center";
    const words = displayLine.split(" ");
    let line = "", lines = [], maxW = W - 64;
    ...
    lines.slice(0, 2).forEach((l, i) => ctx.fillText(l, W / 2, barY + 48 + i * 26));
  }
  ...
  if (script.length > 0 && currentLine >= 0) {
    const dotY = H - 36;
    const dotSpacing = 18;
    const totalW = script.length * dotSpacing;
    const startX = W / 2 - totalW / 2;
    script.forEach((_, i) => {
      ctx.fillStyle = i === currentLine ? "#2ecc71" : i < currentLine ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.12)";
      const dw = i === currentLine ? 20 : 7;
      roundRect(ctx, startX + i * dotSpacing - dw / 2, dotY - 3.5, dw, 7, 3.5);
      ctx.fill();
    });
  }
}

// ─── SVG CHARACTER COMPONENTS (for React UI only) ────────────────────────────
function ConfusedGuy({ speaking = false, size = 120, dim = false }) {
  return (
    <svg width={size} height={size} viewBox="0 0 120 120" fill="none"
      style={{ transition: "filter 0.3s, transform 0.3s", filter: dim ? "brightness(0.45)" : "brightness(1)", transform: speaking ? "scale(1.06)" : "scale(1)" }}>
      <rect x="30" y="70" width="60" height="46" rx="10" fill="#1e3a5f" />
      <rect x="48" y="70" width="11" height="20" rx="2" fill="#ffffff" />
      <rect x="61" y="70" width="11" height="20" rx="2" fill="#ffffff" />
      ...
      <ellipse cx="60" cy="17" rx="27" ry="11" fill="#2c1a0e" />
      <rect x="33" y="17" width="54" height="13" fill="#2c1a0e" />
      <ellipse cx="49" cy="42" rx="6.5" ry={speaking ? 7.5 : 5.5} fill="white" />
      <ellipse cx="71" cy="42" rx="6.5" ry={speaking ? 7.5 : 5.5} fill="white" />
      <circle cx="50.5" cy="43" r="3.2" fill="#1a1a2e" />
      <circle cx="72.5" cy="43" r="3.2" fill="#1a1a2e" />
      <circle cx="51.5" cy="41.5" r="1.1" fill="white" />
      <circle cx="73.5" cy="41.5" r="1.1" fill="white" />
      ...
      <ellipse cx="33" cy="44" rx="5" ry="7.5" fill="#dfa060" />
      <ellipse cx="87" cy="44" rx="5" ry="7.5" fill="#dfa060" />
      <rect x="12" y="72" width="18" height="9" rx="4.5" fill="#1e3a5f" />
      <rect x="90" y="72" width="18" height="9" rx="4.5" fill="#1e3a5f" />
...
function SmartGuy({ speaking = false, size = 120, dim = false }) {
  return (
    <svg width={size} height={size} viewBox="0 0 120 120" fill="none"
      ...
      <rect x="39" y="73" width="3.5" height="14" rx="1.5" fill="#2ecc71" />
      <rect x="45" y="73" width="3.5" height="12" rx="1.5" fill="#e74c3c" />
      <rect x="52" y="57" width="16" height="15" rx="5" fill="#c97c4a" />
      <ellipse cx="60" cy="43" rx="26" ry="28" fill="#c97c4a" />
      <ellipse cx="60" cy="16" rx="26" ry="10" fill="#1a0800" />
      <rect x="34" y="16" width="52" height="12" fill="#1a0800" />
      ...
      <ellipse cx="48.5" cy="42.5" rx="4.5" ry={speaking ? 5.5 : 4} fill="white" />
      <ellipse cx="71.5" cy="42.5" rx="4.5" ry={speaking ? 5.5 : 4} fill="white" />
      <circle cx="49.5" cy="43" r="2.8" fill="#1a1a2e" />
      <circle cx="72.5" cy="43" r="2.8" fill="#1a1a2e" />
      <circle cx="50.5" cy="41.5" r="0.9" fill="white" />
      <circle cx="73.5" cy="41.5" r="0.9" fill="white" />
      ...
      <ellipse cx="34" cy="43" rx="5" ry="7.5" fill="#b36840" />
      <ellipse cx="86" cy="43" rx="5" ry="7.5" fill="#b36840" />
      <rect x="10" y="72" width="18" height="9" rx="4.5" fill="#0f3460" />
      <rect x="92" y="72" width="18" height="9" rx="4.5" fill="#0f3460" />
...
function SubtitleBar({ text, speaker }) {
  if (!text) return null;
  return (
    <div style={{ position: "absolute", bottom: 56, left: 14, right: 14, background: "rgba(8,20,40,0.93)", borderRadius: 14, padding: "10px 16px", textAlign: "center", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.08)", animation: "fadeSlideUp 0.2s ease-out both" }}>
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", gap: 5, padding: "8px 0" }}>
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} style={{ width: i === current ? 22 : 7, height: 7, borderRadius: 3.5, background: i === current ? "#2ecc71" : i < current ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.15)", transition: "all 0.35s ease" }} />
      ))}
    </div>
  );
}
...
function Screen({ children, bg = "#f0f4f8", style = {} }) {
  return (
    <div style={{ minHeight: "100dvh", width: "100%", maxWidth: 430, margin: "0 auto", background: bg, fontFamily: "'Nunito',sans-serif", position: "relative", overflowX: "hidden", animation: "fadeSlideUp 0.3s ease-out both", ...style }}>
      {children}
    </div>
  );
}
...
function TopNav({ onBack, title, light = false }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px 12px", background: light ? "rgba(255,255,255,0.08)" : "transparent" }}>
      ...
      <div style={{ fontSize: 15, fontWeight: 800, color: light ? "#fff" : "#0a2540", letterSpacing: 0.3 }}>{title}</div>
      <div style={{ width: 38 }} />
    </div>
  );
}
...
function LoadingOverlay({ message }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(10,37,64,0.92)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, backdropFilter: "blur(6px)" }}>
      ...
      <div style={{ color: "#2ecc71", fontSize: 15, fontWeight: 800, textAlign: "center", maxWidth: 200 }}>{message}</div>
...
export default function App() {
  ...
  const [approvedQueue, setApprovedQueue] = useState(() => {
    try { return JSON.parse(localStorage.getItem("rat_queue") || "[]"); } catch { return []; }
  });
  ...
  const saveQueue = (q) => {
    setApprovedQueue(q);
    localStorage.setItem("rat_queue", JSON.stringify(q));
  };
  ...
  const fetchNews = useCallback(async (topicId) => {
    setLoading(true); setError(""); setLoadMsg("📡 Fetching latest AI news..."); setTopic(topicId);
    const t = NEWS_TOPICS.find(x => x.id === topicId);
    try {
      let allItems = [];
      for (const feed of RSS_FEEDS.slice(0, 2)) {
        try {
          const res = await fetch(`${PROXY}${encodeURIComponent(feed)}&count=30`);
          const data = await res.json();
          if (data.items?.length) allItems = [...allItems, ...data.items];
        } catch (_) {}
      }
      const keywords = t.query.toLowerCase().split(" ").filter(w => w.length > 3);
      const filtered = allItems.filter(item => {
        ...
        pubDate: item.pubDate ? new Date(item.pubDate).toLocaleDateString("en-GB") : "Recent",
      })).filter(item => item.title?.length > 10);

      setNewsItems(filtered.length ? filtered : allItems.slice(0, 5).map(i => ({
        title: i.title?.trim(), summary: i.description?.replace(/<[^>]+>/g, "").slice(0, 160),
        source: "Tech News", pubDate: "Recent",
      })).filter(i => i.title));
      setScreen("news");
    } catch (e) {
      setError("Could not fetch news. Check your connection.");
    ...
    try {
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "system",
            content: `You write short punchy explainer video scripts for Instagram Reels and TikTok for Rollyadams Techworld — a Nigerian tech company offering Solar/Inverter systems, CCTV Security, and Website & App Development. Two characters: Guy 1 (speaker 0) = confused Lagos businessman, short dramatic questions. Guy 2 (speaker 1) = sharp tech guy, clear simple explanations. English only. 6-8 lines. Max 20 words per line. End with Guy 2 mentioning Rollyadams Techworld naturally.`
          }, {
            role: "user",
            content: `Script for: "${news.title}"\nSummary: "${news.summary || "Latest AI news"}"\n\nReturn ONLY valid JSON array:\n[{"speaker":0,"line":"..."},{"speaker":1,"line":"..."}]`
          }]
        })
      });
      ...
      setError(e.message.includes("401") ? "Invalid Groq API key." : `Error: ${e.message}`);
      setScreen("news");
    } finally { setLoading(false); }
  }, [groqKey]);
  ...
  const speak = useCallback((text, speaker, onEnd) => {
    ...
    utt.rate = speaker === 0 ? 0.88 : 1.0;
    utt.pitch = speaker === 0 ? 0.95 : 1.1;
    utt.volume = 1;
  ...
  const playScript = useCallback(() => {
    ...
    const playLine = () => {
      if (!playingRef.current || idx >= script.length) {
        setPlaying(false); setCurrentLine(-1); setDisplayLine(""); setMouthOpen(false); playingRef.current = false; return;
      }
      setCurrentLine(idx); setDisplayLine(script[idx].line); setMouthOpen(true);
      speak(script[idx].line, script[idx].speaker, () => {
        setMouthOpen(false);
        idx++;
        setTimeout(playLine, 350);
      });
    };
    playLine();
  }, [script, speak]);
  ...
  const stopPlay = useCallback(() => {
    ...
    window.speechSynthesis.cancel();
    setPlaying(false); setCurrentLine(-1); setDisplayLine(""); setMouthOpen(false);
  }, []);
  ...
  const exportVideo = useCallback(async () => {
    ...
    // Animate and speak each line
    let lineIdx = 0;
    const totalLines = script.length;

    // Mouth blink animation
    let mouthState = false;
    let blinkInterval = null;

    const stopBlink = () => { if (blinkInterval) { clearInterval(blinkInterval); blinkInterval = null; } };

    const processLine = (idx) => {
      return new Promise((resolve) => {
        if (idx >= totalLines) { resolve(); return; }

        const line = script[idx];
        setCurrentLine(idx); setDisplayLine(line.line);
        ...
        blinkInterval = setInterval(() => {
          ...
          drawFrame(canvas, script, idx, line.line, selectedNews, topicObj().emoji, mouthState);
        }, 180);

        setExportProgress(Math.round((idx / totalLines) * 90));

        // Speak via Web Speech
        const utt = new SpeechSynthesisUtterance(line.line);
        utt.rate = line.speaker === 0 ? 0.88 : 1.0;
        utt.pitch = line.speaker === 0 ? 0.95 : 1.1;
        const voices = window.speechSynthesis.getVoices();
        const v = voices.find(v => v.lang.startsWith("en")) || voices[0];
        if (v) utt.voice = v;
        ...
        utt.onend = () => {
          stopBlink();
          mouthState = false;
          drawFrame(canvas, script, idx, line.line, selectedNews, topicObj().emoji, false);
  ...
  const approvePost = () => {
    const post = { id: Date.now(), topic: topicObj().label, topicEmoji: topicObj().emoji, news: selectedNews, script, approvedAt: new Date().toLocaleString("en-GB") };
    saveQueue([post, ...approvedQueue]);
    setScreen("approved");
  };
  ...
  useEffect(() => {
    ...
    return () => { window.speechSynthesis.cancel(); if (animFrameRef.current) cancelAnimationFrame(animFrameRef.current); };
  }, []);
  ...
  if (screen === "splash") return (
    <Screen bg="linear-gradient(160deg,#0a1628 0%,#0f2744 50%,#0a2540 100%)">
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100dvh", padding: "0 28px" }}>
        ...
        <h1 style={{ color: "#fff", fontSize: 36, fontWeight: 900, textAlign: "center", margin: "0 0 12px", lineHeight: 1.15, fontFamily: "'Syne','Nunito',sans-serif" }}>AI Content<br />Engine</h1>
    ...
    <Screen>
      ...
      <div style={{ padding: "24px 24px 40px" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <h2 style={{ color: "#0a2540", fontSize: 24, fontWeight: 900, marginBottom: 8, fontFamily: "'Syne',sans-serif" }}>Connect Groq AI</h2>
    ...
    <Screen>
      ...
      <div style={{ padding: "48px 24px 40px" }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <h2 style={{ color: "#0a2540", fontSize: 26, fontWeight: 900, fontFamily: "'Syne',sans-serif" }}>Pick a Topic</h2>
          <p style={{ color: "#64748b", fontSize: 14, marginTop: 4 }}>We'll find the latest AI news for it</p>
        </div>
        <div style={{ display: "grid", gap: 14 }}>
          {NEWS_TOPICS.map(t => (
            <button key={t.id} onClick={() => fetchNews(t.id)} style={{ padding: 20, borderRadius: 16, border: "2px solid #e8f0fe", background: "#fff", cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
              ...
              <div><div style={{ fontSize: 16, fontWeight: 800, color: "#0a2540" }}>{t.label}</div><div style={{ fontSize: 12, color: "#94a3b8", marginTop: 2, fontWeight: 600 }}>Tap to fetch latest news</div></div>
              <div style={{ marginLeft: "auto", color: "#cbd5e1", fontSize: 18 }}>›</div>
            </button>
          ))}
        </div>
        <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
          <button onClick={() => setScreen("setup")} style={{ flex: 1, padding: 12, borderRadius: 12, border: "2px solid #e2e8f0", background: "transparent", color: "#94a3b8", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>⚙️ API Key</button>
  ...
  if (screen === "preview") return (
    <div style={{ minHeight: "100dvh", width: "100%", maxWidth: 430, margin: "0 auto", background: "#08111e", fontFamily: "'Nunito',sans-serif", position: "relative", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, position: "relative", background: "linear-gradient(180deg,#0d1b2e 0%,#0a2540 55%,#081828 100%)", display: "flex", flexDirection: "column" }}>
        ...
        {selectedNews && (
          <div style={{ margin: "0 16px 12px", background: "rgba(46,204,113,0.1)", border: "1px solid rgba(46,204,113,0.25)", borderRadius: 10, padding: "9px 14px", textAlign: "center" }}>
            <div style={{ fontSize: 12, color: "#4ade80", fontWeight: 700, lineHeight: 1.4 }}>{selectedNews.title?.slice(0, 65)}...</div>
          </div>
        )}
      ...
      {exporting && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(8,17,30,0.96)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, zIndex: 999 }}>
          ...
          <div style={{ color: "#2a4a6a", fontSize: 12, textAlign: "center", maxWidth: 220 }}>Keep this screen open and unmute your phone for voice to render</div>
        </div>
      )}
    </div>
  );
  ...
  if (screen === "approved") return (
    <Screen>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100dvh", padding: "0 28px", textAlign: "center" }}>
        ...
        <h2 style={{ color: "#0a2540", fontSize: 28, fontWeight: 900, fontFamily: "'Syne',sans-serif", marginBottom: 8 }}>Approved!</h2>
  ...
  if (screen === "done") return (
    <Screen>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100dvh", padding: "0 28px", textAlign: "center" }}>
        <div style={{ fontSize: 64, marginBottom: 16 }}>🎉</div>
        <h2 style={{ color: "#0a2540", fontSize: 28, fontWeight: 900, fontFamily: "'Syne',sans-serif", marginBottom: 8 }}>Video Ready!</h2>
        <p style={{ color: "#64748b", fontSize: 14, lineHeight: 1.7, marginBottom: 32 }}>Download and post to Instagram Reels or TikTok</p>
        <div style={{ display: "grid", gap: 12, width: "100%" }}>
          {videoURL && (
            <a href={videoURL} download="rollyadams-reel.webm" style={{ display: "block", padding: 17, borderRadius: 14, background: "linear-gradient(135deg,#2ecc71,#1fa355)", color: "#fff", fontSize: 16, fontWeight: 900, textDecoration: "none", boxSizing: "border-box", boxShadow: "0 6px 24px rgba(46,204,113,0.35)" }}>⬇ Download Video</a>
          )}
        ...
        <div style={{ marginTop: 32, background: "#f0f9ff", border: "2px solid #bae6fd", borderRadius: 14, padding: 18, width: "100%", textAlign: "left" }}>
          ...
          {["Post 9–11am or 6–8pm WAT for best reach", "Add 5–8 hashtags in caption", "Pin post to your profile", "Share to your story same day"].map((tip, i) => (
            <div key={i} style={{ fontSize: 13, color: "#0a2540", fontWeight: 600, marginBottom: 6 }}>• {tip}</div>
          ))}
        </div>
      </div>
    </Screen>
    ...
    <Screen>
      ...
      <div style={{ padding: "8px 20px 40px" }}>
        {approvedQueue.length === 0 ? (
          <div style={{ textAlign: "center", paddingTop: 60 }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>📭</div>
            <div style={{ color: "#94a3b8", fontWeight: 700 }}>No approved posts yet</div>
          </div>
        ) : (
          <div style={{ display: "grid", gap: 14 }}>

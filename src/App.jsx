...
const NEWS_TOPICS = [
  ...
  { id: "solar", label: "AI + Solar & Energy", query: "AI solar energy power management Nigeria 2025", emoji: "⚡", color: "#f39c12" },
  { id: "webdev", label: "AI + Web & Apps", query: "AI web development tools automation 2025", emoji: "💻", color: "#3498db" },
  { id: "general", label: "Hot AI News", query: "artificial intelligence latest breakthrough Africa Nigeria 2025", emoji: "🤖", color: "#2ecc71" },
];
...
function drawConfusedGuy(ctx, x, y, size, speaking, dim) {
  ...
  ctx.fillStyle = "#ffffff"; roundRect(ctx, 48, 70, 11, 20, 2); ctx.fill(); roundRect(ctx, 61, 70, 11, 20, 2); ctx.fill();
  ...
  ctx.fillStyle = "#f0b97a"; ctx.beginPath(); ctx.ellipse(60, 44, 27, 29, 0, 0, Math.PI * 2); ctx.fill();
  ...
  if (!speaking) {
    ctx.fillStyle = "white"; ctx.beginPath(); ctx.arc(96, 20, 15, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "#1e3a5f"; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(96, 20, 15, 0, Math.PI * 2); ctx.stroke();
    ctx.fillStyle = "#1e3a5f"; ctx.font = "bold 17px sans-serif"; ctx.textAlign = "center"; ctx.fillText("?", 96, 26);
  }
  ctx.restore();
}
...
function drawSmartGuy(ctx, x, y, size, speaking, dim) {
  ...
  ctx.fillStyle = "#c97c4a"; ctx.beginPath(); ctx.ellipse(60, 43, 26, 28, 0, 0, Math.PI * 2); ctx.fill();
  ...
  if (speaking) {
    ctx.fillStyle = "#fff9e0"; ctx.beginPath(); ctx.arc(96, 18, 15, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = "#f39c12"; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(96, 18, 15, 0, Math.PI * 2); ctx.stroke();
    ctx.font = "16px sans-serif"; ctx.textAlign = "center"; ctx.fillText("💡", 96, 24);
  }
  ctx.restore();
}
...
function drawFrame(canvas, script, currentLine, displayLine, selectedNews, topicEmoji, mouthOpen) {
  ...
  grad.addColorStop(0, "#0d1b2e"); grad.addColorStop(0.5, "#0a2540"); grad.addColorStop(1, "#081828");
  ...
  if (selectedNews?.title) {
    ctx.fillStyle = "rgba(46,204,113,0.12)"; roundRect(ctx, 20, 68, W - 40, 56, 12); ctx.fill();
    ctx.strokeStyle = "rgba(46,204,113,0.3)"; ctx.lineWidth = 1.5; roundRect(ctx, 20, 68, W - 40, 56, 12); ctx.stroke();
    ctx.fillStyle = "#4ade80"; ctx.font = "bold 18px sans-serif"; ctx.textAlign = "center";
    ctx.fillText((topicEmoji + " " + selectedNews.title).slice(0, 55) + "...", W / 2, 102);
  }
  const charY = H * 0.48, charSize = 140;
  const g1Speaking = currentLine >= 0 && script[currentLine]?.speaker === 0 && mouthOpen;
  const g2Speaking = currentLine >= 0 && script[currentLine]?.speaker === 1 && mouthOpen;
  const g1Dim = currentLine >= 0 && script[currentLine]?.speaker === 1;
  const g2Dim = currentLine >= 0 && script[currentLine]?.speaker === 0;
  drawConfusedGuy(ctx, W * 0.27, charY, charSize, g1Speaking, g1Dim);
  drawSmartGuy(ctx, W * 0.73, charY, charSize, g2Speaking, g2Dim);
  ...
  const g2 = ctx.createLinearGradient(W / 2, charY - 40, W / 2, charY + 40);
  g2.addColorStop(0, "transparent"); g2.addColorStop(0.5, "rgba(46,204,113,0.35)"); g2.addColorStop(1, "transparent");
  ctx.strokeStyle = g2; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(W / 2, charY - 40); ctx.lineTo(W / 2, charY + 40); ctx.stroke();
  if (displayLine && currentLine >= 0) {
    const barY = H - 140;
    ctx.fillStyle = "rgba(8,20,40,0.93)"; roundRect(ctx, 16, barY, W - 32, 90, 14); ctx.fill();
    ctx.strokeStyle = "rgba(255,255,255,0.08)"; ctx.lineWidth = 1; roundRect(ctx, 16, barY, W - 32, 90, 14); ctx.stroke();
    const sLabel = script[currentLine]?.speaker === 0 ? "● GUY 1" : "● GUY 2";
    const sColor = script[currentLine]?.speaker === 0 ? "#7dd3fc" : "#4ade80";
    ctx.fillStyle = sColor; ctx.font = "bold 14px sans-serif"; ctx.textAlign = "center"; ctx.fillText(sLabel, W / 2, barY + 22);
    ctx.fillStyle = "#f8fafc"; ctx.font = "bold 20px sans-serif";
    const words = displayLine.split(" "); let line = "", lines = [];
    for (const word of words) { const test = line + word + " "; if (ctx.measureText(test).width > W - 64 && line) { lines.push(line.trim()); line = word + " "; } else line = test; }
    if (line) lines.push(line.trim());
    lines.slice(0, 2).forEach((l, i) => ctx.fillText(l, W / 2, barY + 48 + i * 26));
  }
  if (script.length > 0 && currentLine >= 0) {
    const dotY = H - 36, dotSpacing = 18, totalW = script.length * dotSpacing, startX = W / 2 - totalW / 2;
    script.forEach((_, i) => {
      ctx.fillStyle = i === currentLine ? "#2ecc71" : i < currentLine ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.12)";
      const dw = i === currentLine ? 20 : 7; roundRect(ctx, startX + i * dotSpacing - dw / 2, dotY - 3.5, dw, 7, 3.5); ctx.fill();
    });
  }
}
...
function ConfusedGuy({ speaking = false, size = 120, dim = false }) {
  return (
    <svg width={size} height={size} viewBox="0 0 120 120" fill="none"
      ...
      <rect x="48" y="70" width="11" height="20" rx="2" fill="#fff" />
      <rect x="61" y="70" width="11" height="20" rx="2" fill="#fff" />
...
function SubtitleBar({ text, speaker }) {
  if (!text) return null;
  return (
    <div style={{ position: "absolute", bottom: 56, left: 14, right: 14, background: "rgba(8,20,40,0.93)", borderRadius: 14, padding: "10px 16px", textAlign: "center", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.08)" }}>
...
function Screen({ children, bg = "#f0f4f8", style = {} }) {
  return (
    <div style={{ minHeight: "100dvh", width: "100%", maxWidth: 430, margin: "0 auto", background: bg, fontFamily: "'Nunito',sans-serif", position: "relative", overflowX: "hidden", ...style }}>
      {children}
    </div>
  );
}
...
export default function App() {
  ...
  const saveKey = () => { if (!keyInput.trim()) return; localStorage.setItem(GROQ_KEY_STORAGE, keyInput.trim()); setGroqKey(keyInput.trim()); setScreen("topicSelect"); };

  // ── Fetch News via Groq (no RSS, no CORS issues)
    ...
    try {
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `List 5 real recent AI technology news stories about: "${t.query}". Make them relevant to Nigerian and African businesses. Return ONLY a valid JSON array, no markdown:\n[{"title":"headline here","summary":"2-3 sentence summary","source":"Publication name","pubDate":"Recent"}]\nReturn exactly 5 items with specific realistic headlines.`
          }]
        })
      });
      ...
      setNewsItems(parsed.filter(i => i.title?.length > 10));
      setScreen("news");
    } catch (e) {
      setError(e.message.includes("401") ? "Invalid Groq API key. Please check and re-enter." : `Error: ${e.message}`);
  ...
  const exportVideo = useCallback(async () => {
    ...
    recorder.onstop = () => {
      ...
      setExportProgress(100); setExporting(false); setScreen("done");
    };
  }, [script, selectedNews, topic]);

  const approvePost = () => {
    saveQueue([{ id: Date.now(), topic: topicObj().label, topicEmoji: topicObj().emoji, news: selectedNews, script, approvedAt: new Date().toLocaleString("en-GB") }, ...approvedQueue]);
    setScreen("approved");
  };
  ...
  if (screen === "splash") return (
    <Screen bg="linear-gradient(160deg,#0a1628 0%,#0f2744 50%,#0a2540 100%)">
      ...
      <style>{`@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}} @keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:0.7;transform:scale(0.97)}} @keyframes fadeSlideUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}`}</style>
    </Screen>
  );
